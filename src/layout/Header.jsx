  ...
  if (!userInfo) {
    ...
    return (
        ...
        <div className='flex justify-center items-center gap-8 relative'>
          {userInfo.role === 'seller' && <SellerNotifications />}
          <div className='flex justify-center items-center'>
            <div className='flex justify-center items-center gap-3'>
              <div className='flex justify-center items-center flex-col text-end'>
          <h2 className='text-md font-bold'>{ userInfo.name }</h2>
          <span className='text-[14px] w-full font-normal'>{ userInfo.role === 'seller' ? 'Vendeur' : userInfo.role === 'admin' ? 'Administrateur' : userInfo.role }</span>
              </div>

              {
                userInfo.role === 'admin' ? <img className='w-[50px] h-[50px] rounded-full overflow-hidden' src="http://localhost:3001/images/logo.svg" alt="" />  : <img className='w-[50px] h-[50px] rounded-full overflow-hidden' src={userInfo.image} alt="" />
              }
